...
def leaky_relu(features, alpha=0.2, name=None):
    with ops.name_scope(name, "LeakyRelu", [features, alpha]):
        ...
        alpha = ops.convert_to_tensor(
            alpha, dtype=features.dtype, name="alpha")
...
def sndeconv_layer(x, filter_shape, output_shape, stride, stddev=0.02, update_collection=None):
    ...
    filters = tf.get_variable(
        ...
        initializer=tf.truncated_normal_initializer(stddev=stddev),
        trainable=True)
    return tf.nn.conv2d_transpose(x, sn.spectral_normed_weight(filters, update_collection=update_collection),
